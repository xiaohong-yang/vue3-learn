<template>
  ...
  <div>msg：{{ msgToChild }}</div>
  <!-- <h3>count:{{ count }}</h3> -->
  <el-button @click="changeMsgByChild">分发事件(子组件 => 父组件)</el-button>
</template>
...
export default defineComponent ({
  ...
  setup (props, { attrs, slots, emit }) {
    ...
    const changeMsgByChild = () => {
      // context.emit('msgToParent', '，猪比' )
      emit('msgToParent', '，猪比' )
    }
    return {
      showMsg1,
